...
[[ -z "$download_dir" || "$download_dir" == "null" ]] && failure $EXIT_INVALID_CONFIG "Download directory cannot be null or empty."
[[ -z "$default_username" || "$default_username" == "null" ]] && failure $EXIT_INVALID_CONFIG "Default username cannot be null or empty."
[[ -z "$default_password" || "$default_password" == "null" ]] && failure $EXIT_INVALID_CONFIG "Default password cannot be null or empty."
[[ -z "$location" || "$location" == "null" ]] && failure $EXIT_INVALID_CONFIG "Location cannot be null or empty."
[[ -z "$data_type" || "$data_type" == "null" ]] && failure $EXIT_INVALID_CONFIG "Data type cannot be null or empty."
[[ -z "$bandwidth_limit" || "$bandwidth_limit" == "null" ]] && failure $EXIT_INVALID_CONFIG "Bandwidth limit cannot be null or empty."
[[ -z "$num_meters" || "$num_meters" -eq 0 ]] && failure $EXIT_INVALID_CONFIG "Must have at least 1 meter in the config file."

# Create the base output directory
output_dir="$download_dir/$location/$data_type"
mkdir -p "$output_dir" && log "Directory created successfully: $output_dir" || failure $EXIT_DIR_CREATION_FAIL "Failed to create directory: $output_dir"
...
for ((i = 0; i < num_meters; i++)); do
    ...
    if "$current_dir/meters/$meter_type/download.sh" "$meter_ip" "$output_dir" "$meter_id" "$meter_type" "$bandwidth_limit" "$data_type" "$location"; then
        ...
        failure $EXIT_DOWNLOAD_FAIL "Download incomplete for meter: $meter_id"
    fi
done
