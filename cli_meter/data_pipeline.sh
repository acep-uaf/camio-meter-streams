...
[[ -z "$location" ]] && fail $EXIT_CONFIG_NULL_VALUES "Location cannot be null or empty"
[[ -z "$data_type" ]] && fail $EXIT_CONFIG_NULL_VALUES "Data type cannot be null or empty"
[[ -z "$bw_limit" ]] && fail $EXIT_CONFIG_NULL_VALUES "Bandwith limit cannot be null or empty"
...
if [ $? -eq 0 ]; then
    ...
    fail $EXIT_DIR_NOT_EXIST "Failed to create directory: $output_dir"
fi
...
for ((i = 0; i < num_meters; i++)); do
    ...
    if "$current_dir/meters/$meter_type/download.sh" "$meter_ip" "$output_dir" "$meter_id" "$meter_type" "$bw_limit" "$data_type" "$location"; then
        ...
        fail $EXIT_UNKNOWN "Download failed for meter: $meter_id"
    fi
done
