...
[[ -z "$dest_host" || "$dest_host" == "null" ]] && failure $EXIT_INVALID_CONFIG "Destination host cannot be null or empty."
[[ -z "$dest_user" || "$dest_user" == "null" ]] && failure $EXIT_INVALID_CONFIG "Destination user cannot be null or empty."
[[ -z "$ssh_key_path" || "$ssh_key_path" == "null" ]] && failure $EXIT_INVALID_CONFIG "SSH key path cannot be null or empty."
...
for i in $(seq 0 $((num_dirs - 1))); do
    ...
    [[ -z "$src_dir" ]] && failure $EXIT_INVALID_CONFIG "Source directory cannot be null or empty."
    [[ -z "$dest_dir" ]] && failure $EXIT_INVALID_CONFIG "Destination directory cannot be null or empty."
    ...
    if [ -d "$src_dir" ] && [ -n "$(ls -A "$src_dir")" ]; then
        ...
        [ $rsync_status -eq 0 ] && log "Data transfer from $src_dir to $dest_host completed successfully" || failure $EXIT_RSYNC_FAIL "Data synchronization from $src_dir to $dest_host failed"
    else
        failure $EXIT_DIR_NOT_FOUND "Source directory $src_dir doesn't exist or is empty"
    fi
done
